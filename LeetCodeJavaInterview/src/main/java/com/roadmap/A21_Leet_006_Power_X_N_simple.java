 ...
 * -104 <= xn <= 104
 */
public class A21_Leet_006_Power_X_N_simple {

    public static void main(String[] args) {
        A21_Leet_006_Power_X_N_simple leet = new A21_Leet_006_Power_X_N_simple();
    ...
    private double pow(double base, int powerOf) {
        if (powerOf == 0 || base == 1.0) return 1.0;

        long exp = powerOf < 0 ? -(long) powerOf : powerOf;
        double result = 1.0;

        for (double factor = base; exp > 0; exp >>= 1, factor *= factor) {
            if ((exp & 1L) == 1L) result *= factor;
        }
        return powerOf < 0 ? 1.0 / result : result;
    }
