 ...
 * -104 <= xn <= 104
 */
public class A21_Leet_006_Power_X_N_faster {

    public static void main(String[] args) {
        A21_Leet_006_Power_X_N_faster leet = new A21_Leet_006_Power_X_N_faster();
...
//        System.out.println(leet.myPow(1.0, 2147483647));// 1.0
    ...
    public double myPow(double x, int n) {
        ...
        double valor = 1.0;
        while (power != 0) {
            //n is a odd number, so i must do a least one normal interaction before we can divide by 2 (split in half)
            if (power % 2 == 1) {
                //
                valor = valor * x;
                power--;
            }
            //Now we can multiply x by x, and not valor by x, obs: the last interaction will always be One (odd number), the value of the operation will be set to x.
            x = x * x;
            power = power / 2;
        }
