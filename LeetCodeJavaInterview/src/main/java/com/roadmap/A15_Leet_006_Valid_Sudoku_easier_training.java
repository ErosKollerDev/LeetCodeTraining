 ...
 */
public class A15_Leet_006_Valid_Sudoku_easier_training {
    public static void main(String[] args) {
        A15_Leet_006_Valid_Sudoku_easier_training leet = new A15_Leet_006_Valid_Sudoku_easier_training();
        char[][] sudoku = new char[][]
                {{'5', '3', '.', '.', '7', '.', '.', '.', '.'}
                        ...
                        , {'.', '.', '.', '4', '1', '9', '.', '.', '5'}
                        , {'.', '.', '.', '.', '8', '.', '.', '7', '9'}};
        boolean validSudoku = leet.isValidSudoku(sudoku);
        ...
        validSudoku = leet.isValidSudoku(sudoku);
        System.out.println("Is sudoku valid: " + validSudoku);
    ...
    public boolean isValidSudoku(char[][] board) {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                var c = board[i][j];
                if (c == '.') continue;
                var rowId = "rowId %d - %c".formatted(i, c);
                var colId = "colId %d - %c".formatted(j, c);
                var sqdId = "sqdId %d - %d - %c".formatted(i / 3, j / 3, c);
                if (!seenBefore.add(rowId) || !seenBefore.add(colId) || !seenBefore.add(sqdId)) {
                    return false;
                }
            }
        }
